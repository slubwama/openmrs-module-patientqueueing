...
public interface PatientQueueingService extends OpenmrsService {
	 ...
	 */
	@Transactional(readOnly = true)
	PatientQueue getPatientQueueByTicketNumber(String ticketNumberOrVisitNumber, Date fromDate, Date toDate);
	
	/**
	 * Mark a queue entry as PRESENT (checked-in).
	 */
	@Transactional
	PatientQueue checkInByTicketNumber(String ticketNumberOrVisitNumber, Location facilityLocation, String deviceId);
}
