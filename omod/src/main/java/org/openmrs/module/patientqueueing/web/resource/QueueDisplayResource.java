...
import org.openmrs.module.patientqueueing.web.dto.QueueDisplayContextDto;
import org.openmrs.module.patientqueueing.web.dto.QueueDisplayDto;
import org.openmrs.module.patientqueueing.web.dto.QueueDisplayRowDto;
import org.openmrs.module.patientqueueing.web.dto.QueueDisplayStatsDto;
...
public class QueueDisplayResource extends DelegatingCrudResource<QueueDisplayDto> {
	...
	private QueueDisplayDto buildFromParams(RequestContext context) throws ResponseException {
		...
		if ("facility".equalsIgnoreCase(type)) {
			active = service().getPatientQueueByParentLocation(loc, null, range.from, range.to, false);
			return buildDisplayPayload("FACILITY", loc, active);
		} else if ("location".equalsIgnoreCase(type)) {
			active = service().getPatientQueueList(null, range.from, range.to, loc, null, null, null, null);
			return buildDisplayPayload("LOCATION", loc, active);
		} else if ("room".equalsIgnoreCase(type)) {
			active = service().getPatientQueueList(null, range.from, range.to, null, null, null, null, loc);
			return buildDisplayPayload("ROOM", loc, active);
		} else {
			throw new UnsupportedOperationException("type must be facility|location|room");
		}
	}
	
	private QueueDisplayDto buildDisplayPayload(String type, Location contextLocation, List<PatientQueue> items) {
